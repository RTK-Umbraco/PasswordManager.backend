...
internal class OperationService : IOperationService
{
    ...
    {
        _logger.LogTrace("Getting all operations for SecurityKey: {SecurityKeyId}", SecurityKeyId);

        var operations = await _operationRepository.GetSecurityKeyOperations(SecurityKeyId);

        _logger.LogTrace("Found {Count} operations for SecurityKey: {SecurityKeyId}", operations.Count, SecurityKeyId);
        return operations;
    }
}
