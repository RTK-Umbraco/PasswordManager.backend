...
internal class OperationService : IOperationService
{
    ...
    {
        _logger.LogTrace("Getting all operations for Password: {PasswordId}", PasswordId);

        var operations = await _operationRepository.GetPasswordOperations(PasswordId);

        _logger.LogTrace("Found {Count} operations for Password: {PasswordId}", operations.Count, PasswordId);
        return operations;
    }
}
